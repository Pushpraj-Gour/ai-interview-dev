...
async def write_to_json(data, file_name):
    ...
    with open(filepath_with_name, 'w', encoding='utf-8') as f:
        ...
        logger.info(f"Wrote {file_name} at {filepath_with_name}")
...
async def generate_initial_questions(candidate_info: Dict):
    ...
    try:
        ...
        logger.error(f"An error occurred while gathering candidate information: {str(e)}")
    ...
    logger.info(f"First question to ask: {question_to_ask}")
...
async def process_audio_response(question:str, audio_file: bytes):
    ...
    try:
        ...
        logger.info("Audio file loaded into memory successfully.")

    except Exception as e:
        logger.error(f"Failed to initialize ElevenLabs or load audio.")
    ...
    try:
        ...
        logger.error(f"Audio transcription failed after retries.")
        raise ValueError(f"An error occurred while processing the audio file: {str(e)}")
    
    if not response or not getattr(response, "text", None):
        logger.error("Failed to transcribe the audio file.")
        raise HTTPException(
            status_code=500,
    ...
    logger.info(f"Transcription for question '{question}' completed successfully.")
...
async def next_question():
    ...
    try:
        ...
        if LAST_QUESTION_ASKED_IS_FOLLOW_UP:
            ...
            if not MAJOR_QUESTIONS:
                logger.error("No more major questions available after follow-up.")
            ...
            logger.info(f"Next question to ask after follow-up: {question_to_ask}")
            return question_to_ask


        if not CANDIDATE_INFO:
            logger.error("Candidate information is not provided, cannot generate next question.")
            raise HTTPException(status_code=400, detail="Candidate information is not provided.")
        
        if not MAJOR_QUESTIONS:
            logger.error("No more major questions available.")
        ...
        if not output:
            logger.error("No output received from LLM for next question generation.")
            raise HTTPException(
                status_code=500,
                detail="Failed to generate the next question. Please try again later."
            )
        logger.info(f"LLM output for next question: {output}")
    ...
    try:
        ...
        if decision == "follow_up":
            ...
            if not follow_up_question:
                logger.error("Follow-up question is not provided in the output of llm call.")
            ...
            logger.info(f"Follow-up question to ask: {follow_up_question}")
            ...
            if major_question_selected not in MAJOR_QUESTIONS:
                logger.warning(f"Selected major question '{major_question_selected}' has already been asked or does not exist.")
    ...
    # If anything goes wrong, we will select a question from the major questions that have not been asked yet
        ...
        logger.exception(f"Error processing LLM output for next question: {str(e)}")

        if not MAJOR_QUESTIONS:
            logger.error("No more major questions available after processing LLM output.")
        ...
        logger.info(f"Fallback major question selected: {fallback_question}")
...
async def generate_feedback():
    ...
    if not CANDIDATE_INFO or "role" not in CANDIDATE_INFO:
        logger.error("Candidate role is missing in candidate information.")
        raise HTTPException(status_code=400, detail="Candidate role is required.")

    if not QUESTION_ANSWERED:
        logger.error("No questions have been answered by the candidate.")
    ...
    try:
        ...
        if not output:
            logger.error("No feedback returned by LLM.")
            raise HTTPException(
                status_code=500,
                detail="Failed to generate feedback. Please try again later."
            )
        
        feedback_by_question = output
        ...
        if not overall_analysis:
            logger.error("No overall analysis returned by LLM.")
            raise HTTPException(
                status_code=500,
        ...
        logger.error(f"An error occurred while generating feedback: {str(e)}")
...
async def process_and_give_overall_feedback(per_question_analysis):
    ...
    try:
        ...
        if not question_analysis or not isinstance(question_analysis, list):
            logger.error("Invalid or missing 'question_analysis' in input.")
        ...
        logger.error(f"An error occurred while processing overall feedback: {str(e)}")
